                ...
                <div className="lg:col-span-2 space-y-8">
                                ...
                                <div key={app.id} className="flex items-center justify-between p-4 bg-black/20 border border-zinc-800 hover:border-zinc-700 transition-colors">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-[10px] text-zinc-500 font-mono mb-1">{app.email}</div>
                                            <div className="flex gap-3 text-[10px] text-zinc-600">
                                                {app.phone && (
                                                    <span title="Phone">📞 {decrypt(app.phone)}</span>
                                                )}
                                                {app.dateOfBirth && (
                                                    <span title="DOB">🎂 {new Date(app.dateOfBirth).toLocaleDateString()}</span>
                                                )}
                                            </div>
                    ...
                    <div className="bg-zinc-900 border border-zinc-800 p-6 space-y-4">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <Link href="/admin/exams" className="text-[10px] font-black uppercase tracking-widest text-primary hover:text-white transition-colors">
                                View All
                            </Link>
                        </div>
                        <DataExport
                            ...
                            <table className="w-full text-left">
                                ...
                                <tbody className="divider-y divider-zinc-800">
                                    {ongoingApplications.map((app: any) => (
                                        <tr key={app.id} className="border-b border-zinc-800/50 hover:bg-white/5 transition-colors group">
                                            <td className="p-4">
                                                <div className="font-bold text-white text-xs uppercase">{app.student.user.name}</div>
                                                <div className="text-[10px] text-zinc-600">ID: {app.student.admissionNumber}</div>
                                            </td>
                                            <td className="p-4">
                                                <div className="text-xs font-medium text-zinc-300">{app.template.title}</div>
                                                <div className="text-[10px] text-zinc-600">{new Date(app.template.date).toLocaleDateString()}</div>
                                            </td>
                                            <td className="p-4">
                                                <span className={`text-[10px] font-black uppercase px-2 py-1 rounded-sm ${app.status === 'APPROVED' ? 'bg-emerald-500/10 text-emerald-500' :
                                            ...
                                            <td className="p-4">
                                                <span className={`text-[10px] font-bold uppercase ${app.paymentStatus === 'PAID' ? 'text-emerald-500' : 'text-zinc-500'
                                                    }`}>
                                                    {app.paymentStatus}
                                                </span>
                                            </td>
                                            <td className="p-4 text-right">
                                                <Link href={`/admin/exams/review/${app.id}`} className="text-[10px] font-black uppercase tracking-widest text-primary opacity-0 group-hover:opacity-100 transition-all">
                                                    Review
                                                </Link>
                                            </td>
                                        </tr>
                ...
                <div className="space-y-8">
                    ...
                    <div className="bg-zinc-900 border border-zinc-800 p-6 space-y-4">
                        ...
                        <div className="space-y-2">
                            {classSchedules.map((schedule: any) => (
                                <div key={schedule.id} className="flex items-center justify-between p-3 bg-black/20 border border-zinc-800">
                                    <div>
                                        <div className="text-xs font-bold text-white uppercase">{schedule.name}</div>
                                        <div className="text-[10px] text-zinc-500">{schedule.day} @ {schedule.startTime}</div>
                                    </div>
                                    <div className="text-xs font-bold text-zinc-600">
                                        {schedule._count.students} <span className="text-[10px] font-normal">Students</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="bg-zinc-900 border border-zinc-800 p-6 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <Link href="/admin/students" className="p-4 bg-black/20 border border-zinc-800 hover:border-zinc-600 hover:bg-zinc-800/50 transition-all group text-center">
                                <Users className="mx-auto mb-2 text-zinc-500 group-hover:text-primary transition-colors" size={20} />
                                <div className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Students</div>
                            </Link>
                            <Link href="/admin/curriculum" className="p-4 bg-black/20 border border-zinc-800 hover:border-zinc-600 hover:bg-zinc-800/50 transition-all group text-center">
                                <TrendingUp className="mx-auto mb-2 text-zinc-500 group-hover:text-primary transition-colors" size={20} />
                                <div className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Curriculum</div>
                            </Link>
                            <Link href="/admin/content" className="p-4 bg-black/20 border border-zinc-800 hover:border-zinc-600 hover:bg-zinc-800/50 transition-all group text-center">
                                <PenTool className="mx-auto mb-2 text-zinc-500 group-hover:text-primary transition-colors" size={20} />
                                <div className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Content</div>
                            </Link>
                            <Link href="/admin/settings" className="p-4 bg-black/20 border border-zinc-800 hover:border-zinc-600 hover:bg-zinc-800/50 transition-all group text-center">
                                <Trophy className="mx-auto mb-2 text-zinc-500 group-hover:text-primary transition-colors" size={20} />
                                <div className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Ranks</div>
                            </Link>
                        </div>
                    </div>
                </div>
