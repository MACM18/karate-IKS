...
export default async function AdminCurriculumPage() {
  ...
  const ranksWithStats = ranks.map((rank) => {
    const categoryCounts = rank.curriculumItems.reduce(
  ...
  const totalKata = ranksWithStats.reduce(
    (sum, r) => sum + (r.categoryCounts.KATA || 0),
    0,
  );
  const totalTech = ranksWithStats.reduce(
    (sum, r) => sum + (r.categoryCounts.TECHNIQUE || 0),
    0,
  );
  const totalKumite = ranksWithStats.reduce(
  ...
  return (
    <div className='p-8 space-y-12 animate-in fade-in duration-500'>
      ...
      <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
        {[
          {
            label: "KATA PROTOCOLS",
            value: totalKata,
            color: "text-orange-500",
            icon: BookOpen,
          },
          {
            label: "TECHNIQUE SETS",
            value: totalTech,
            color: "text-blue-500",
            icon: HandMetal,
          },
          ...
          <div
            key={i}
            className='bg-zinc-900/50 border border-zinc-800 p-6 flex flex-col justify-between group hover:border-zinc-700 transition-all'
          >
            <div className='flex justify-between items-start mb-4'>
              <stat.icon className={`w-5 h-5 ${stat.color} opacity-70`} />
              <span
                className={`text-xs font-black bg-zinc-950 px-2 py-1 rounded text-zinc-500`}
              >
                SYS.0{i + 1}
              </span>
            </div>
                ...
                {rank.totalItems > 0 ? (
                  <div className='grid grid-cols-2 gap-2 pt-2 border-t border-zinc-900'>
                    ...
                    {rank.categoryCounts.TECHNIQUE && (
                      <div className='flex items-center gap-2 text-[10px] text-zinc-400 uppercase font-bold'>
                        <HandMetal size={10} className='text-blue-500' />{" "}
                        {rank.categoryCounts.TECHNIQUE} Tech
                      </div>
                    )}
                    {rank.categoryCounts.KUMITE && (
                      <div className='flex items-center gap-2 text-[10px] text-zinc-400 uppercase font-bold'>
                        <Swords size={10} className='text-green-500' />{" "}
