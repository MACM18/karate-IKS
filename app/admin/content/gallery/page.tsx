...
import { Plus, Trash2, Star, Image as ImageIcon, Tag, Folder } from 'lucide-react';
    ...
    return (
        <div className="p-8 space-y-12 max-w-full mx-auto min-h-screen relative pb-32">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b border-zinc-900">
                <div>
                    <h1 className="text-5xl font-heading font-black uppercase tracking-tighter text-white">Visual <span className="text-primary italic">Gallery</span></h1>
                    <p className="text-zinc-500 mt-2 font-medium">Manage library imagery, create albums, and organize visual assets.</p>
                </div>
            ...
            {categories.length === 0 ? (
                <div className="py-24 text-center border-2 border-dashed border-zinc-900 rounded-2xl text-zinc-700 bg-zinc-950/50">
                    ...
                    <p className="text-sm mt-2 opacity-60">Upload your first visual intel to start a category.</p>
                </div>
            ) : (
                        ...
                        <div key={category} className="space-y-6">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest">{groupedItems[category].length} Assets</p>
                                </div>
                            </div>

                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                                {groupedItems[category].map((item) => (
                                    <div key={item.id} className="group relative aspect-[4/3] bg-zinc-900 border border-zinc-900 rounded-xl overflow-hidden hover:border-primary/50 hover:shadow-2xl hover:shadow-primary/10 transition-all duration-500">
                                        <img
                                            src={item.url}
                                        ...
                                        <div className="absolute inset-0 p-4 flex flex-col justify-between translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                                            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity delay-100">
                                                <form action={async () => {
                                                    ...
                                                    <button className={`p-2 rounded-lg backdrop-blur-md border border-white/10 transition-colors ${item.featured ? 'bg-yellow-500/20 text-yellow-500 border-yellow-500/50' : 'bg-black/50 text-white hover:bg-white hover:text-black'}`}>
                                            ...
                                            <div>
                                                <h3 className="text-sm font-bold text-white leading-tight mb-1 drop-shadow-md">{item.title}</h3>
                                                {item.caption && (
                                                    <p className="text-[10px] text-zinc-400 line-clamp-2 font-medium leading-relaxed">{item.caption}</p>
                                                )}
                                            </div>
                                        </div>
