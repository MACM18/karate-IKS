...
export default async function StudentDashboard() {
        ...
        return (
            <div className="min-h-screen bg-black text-white flex items-center justify-center p-8">
                <div className="text-center">
                    ...
                    <p className="text-zinc-500">Student credentials required for this area.</p>
                </div>
            </div>
        );
    }
    ...
    const appliedTemplateIds = new Set(profile.applications.map((a: any) => a.templateId));
    const availableExams = activeExams.filter((e: any) => !appliedTemplateIds.has(e.id));
    ...
    const maskedPhone = profile.phone ? maskData(decrypt(profile.phone)) : "Not provided";
    const maskedEmergency = profile.emergencyContact ? maskData(decrypt(profile.emergencyContact)) : "Not provided";
                    ...
                    <div className="flex flex-col md:flex-row items-end gap-8">
                        ...
                        <div className="flex-1 mb-2">
                            <h1 className="text-6xl md:text-8xl font-heading font-black uppercase tracking-tighter leading-none mb-6">
                                {session.user.name?.split(" ")[0]} <span style={{ color: themeColor }}>{session.user.name?.split(" ").slice(1).join(" ")}</span>
                            </h1>
                            <div className="flex flex-wrap gap-6 text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500">
                                <div className="flex items-center gap-2">
                                    <MapPin size={16} className="text-zinc-600" /> Shito-Ryu Shinbukan HQ
                                </div>
                                <div className="flex items-center gap-2">
                                    <Calendar size={16} className="text-zinc-600" /> Enrolled {new Date(profile.createdAt).toLocaleDateString(undefined, { month: 'long', year: 'numeric' })}
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                                <div className="max-w-2xl text-center md:text-left">
                                    ...
                                    <p className="text-zinc-400 text-sm leading-relaxed font-medium">
                                        The Sensei Council has opened applications for the upcoming grading cycle. This is your chance to advance along the Path of Power. Review your requirements and submit your application before the deadline.
                                    </p>
                                </div>
                                <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
                                    {availableExams.map((exam: any) => (
                                        ...
                                        >
                                            <span className="skew-x-[12deg] inline-block">Apply: {exam.title}</span>
                                        </Link>
                                    ))}
                                </div>
                            </div>
                    ...
                    <div className="lg:col-span-2 space-y-16">
                        <section>
                            ...
                            <div className="bg-zinc-900/20 border border-zinc-800/50 p-10 rounded-3xl">
                                <BeltProgress
                                    ...
                                    progress={Math.min(Math.round((totalClasses / 24) * 100), 100)}
                                    color={themeColor}
                                />
                            </div>
                        </section>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="p-10 bg-zinc-900/20 border border-zinc-800 rounded-3xl hover:bg-zinc-900/40 transition-all group border-b-4 border-b-zinc-800 hover:border-b-[var(--belt-theme)] translate-y-0 hover:-translate-y-1">
                                ...
                                <h3 className="font-heading uppercase text-3xl font-black mb-3">Curriculum</h3>
                                <p className="text-sm text-zinc-500 mb-8 leading-relaxed font-medium italic">Master the Heian Katas and Kihon Kumite required for your current rank phase.</p>
                                <Link href="/student/resources" className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--belt-theme)] hover:brightness-125 flex items-center gap-2">
                                    Open Archives <div className="w-10 h-px bg-[var(--belt-theme)] opacity-40" />
                                </Link>
                            </div>
                            ...
                            <div className="p-10 bg-zinc-900/20 border border-zinc-800 rounded-3xl hover:bg-zinc-900/40 transition-all group border-b-4 border-b-zinc-800 hover:border-b-amber-500 translate-y-0 hover:-translate-y-1">
                                ...
                                <h3 className="font-heading uppercase text-3xl font-black mb-3">Honors</h3>
                                ...
                                {profile.achievements.length > 0 && (
                                    <div className="space-y-3">
                                        {profile.achievements.slice(0, 3).map((a: any) => (
                                            <div key={a.id} className="text-[10px] font-black uppercase tracking-widest text-zinc-400 bg-white/5 p-3 rounded-xl flex items-center justify-between border border-zinc-800/50">
                                                <span>★ {a.title}</span>
                                                <span className="text-[8px] opacity-40">{new Date(a.date).getFullYear()}</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
                    ...
                    <aside className="space-y-12">
                        <div className="bg-zinc-900/30 p-10 border border-zinc-800 rounded-3xl backdrop-blur-sm">
                            <h3 className="text-zinc-500 uppercase text-[10px] font-black tracking-[0.3em] mb-10 flex items-center justify-between">
                                Security Archive <ShieldAlert size={14} className="opacity-20" />
                            </h3>
                            <div className="space-y-8">
                                <div className="group cursor-default">
                                    <div className="text-[8px] font-black text-zinc-700 uppercase tracking-widest mb-1 group-hover:text-[var(--belt-theme)] transition-colors">Combat Identity</div>
                                    <div className="text-base font-black text-white uppercase tracking-tighter">{session.user.name}</div>
                                </div>
                                <div className="group cursor-default">
                                    <div className="text-[8px] font-black text-zinc-700 uppercase tracking-widest mb-1 group-hover:text-[var(--belt-theme)] transition-colors">Encrypted Signal</div>
                                    <div className="text-sm font-bold text-zinc-400 font-mono tracking-tighter bg-black/40 p-2 rounded border border-zinc-800/50">{maskedPhone}</div>
                                </div>
                                <div className="group cursor-default">
                                    <div className="text-[8px] font-black text-zinc-700 uppercase tracking-widest mb-1 group-hover:text-[var(--belt-theme)] transition-colors">Emergency Bridge</div>
                                    <div className="text-sm font-bold text-zinc-400 font-mono tracking-tighter bg-black/40 p-2 rounded border border-zinc-800/50">{maskedEmergency}</div>
                                </div>
                                <div className="pt-8 border-t border-zinc-800/50">
                                    <button className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-500 hover:text-white transition-all flex items-center gap-2 group">
                                        <User size={12} className="group-hover:text-[var(--belt-theme)]" /> Modify Intelligence Profile
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-10 bg-black border border-zinc-800 rounded-3xl relative overflow-hidden group hover:border-[var(--belt-theme)] transition-all">
                            ...
                            <h3 className="text-xl font-heading font-black uppercase mb-3 relative z-10 italic">Secure Line</h3>
                            <p className="text-xs text-zinc-500 leading-relaxed mb-8 relative z-10 font-medium">Encountering structural discrepancies in your progression data? Initiate contact with HQ immediately.</p>
