  ...
  if (params.id) {
    ...
    return (
      <div className='p-4 md:p-12 max-w-4xl mx-auto'>
        <Link
          href='/student/exams'
          className='text-xs font-black uppercase tracking-widest text-zinc-500 hover:text-primary mb-8 inline-block'
        >
    ...
    <div className='p-4 md:p-8 space-y-12 max-w-7xl mx-auto'>
      <header>
        <h1 className='text-5xl font-heading font-black uppercase tracking-tighter text-white'>
          Grading <span className='text-primary italic'>Portal</span>
        </h1>
        <p className='text-zinc-500 mt-2 font-medium'>
          Apply for rank progression and school events.
        </p>
      </header>
          ...
          <div className='grid grid-cols-1 gap-4'>
            {templates.map((template) => {
              const isPastDeadline =
                template.deadline && new Date(template.deadline) < new Date();

              return (
                <div
                  key={template.id}
                  className={`bg-zinc-900 border border-zinc-800 p-6 flex flex-col md:flex-row md:items-center justify-between group transition-all gap-4 ${
                    isPastDeadline
                ...
                >
                  <div className='space-y-1'>
                    <div className='flex items-center gap-3'>
                      <h3
                        className={`text-xl font-bold text-white uppercase tracking-tight transition-colors ${
                          !isPastDeadline && "group-hover:text-primary"
                        }`}
                      >
                        {template.title}
                      </h3>
                    ...
                    {template.deadline && (
                      <p
                        className={`text-[10px] font-black uppercase tracking-widest mt-2 ${
                          isPastDeadline ? "text-red-500" : "text-zinc-400"
                        }`}
                      >
                        Deadline:{" "}
                        {new Date(template.deadline).toLocaleDateString()}{" "}
        ...
        <aside className='space-y-8'>
          ...
          <div className='space-y-4'>
            {myApplications.map((app) => (
              <div
                key={app.id}
                className='bg-zinc-900/50 border border-zinc-800 p-5 space-y-3'
              >
