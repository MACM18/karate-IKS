...
export default async function Home() {
  ...
  const displayBlackBelts =
    blackBeltCount > 0 ? blackBeltCount.toString() : "12";

  const instructors = senseis.map((sensei) => ({
    name: sensei.name || "Sensei",
    role: sensei.studentProfile?.currentRank?.name || "Instructor",
    bio: sensei.studentProfile?.bio || "Dedicated instructor at IKS.",
    image:
      sensei.image ||
  ...
  return (
    <main className='bg-background min-h-screen'>
      ...
      {session && activeExamsFetch.length > 0 && (
          ...
          <div className='max-w-7xl mx-auto px-4 md:px-8'>
            <div className='flex flex-col md:flex-row md:items-center justify-between gap-6'>
              <div className='flex items-center gap-4'>
                ...
                <div>
                  <h2 className='text-xl font-heading uppercase text-foreground tracking-widest leading-none'>
                    Dojo Alerts
                  </h2>
                  <p className='text-zinc-500 text-sm mt-1 uppercase font-black tracking-widest text-[10px]'>
                    Active Exams & Registrations
                  </p>
                </div>
              </div>
              <div className='flex flex-wrap gap-4'>
                {activeExamsFetch.map((exam) => (
                  <Link
                    key={exam.id}
                  ...
                  >
                    <div className='text-left'>
                      <div className='text-sm font-bold text-white uppercase group-hover:text-primary'>
                        {exam.title}
                      </div>
                      {exam.deadline && (
                    ...
                    <ArrowRight
                      size={16}
                      className='text-zinc-700 group-hover:text-primary transition-transform group-hover:translate-x-1'
                    />
                  </Link>
                ))}
              </div>
      ...
      {/* Latest Intelligence Section - Default Background */}
      <section className='w-full py-32 bg-background text-foreground'>
        <div className='max-w-7xl mx-auto px-4 md:px-8'>
      ...
      {/* Visual Legacy Snippet - Muted Background */}
      <section className='w-full py-32 bg-muted/30 text-foreground border-t border-border'>
        <div className='max-w-7xl mx-auto px-4 md:px-8'>
      ...
      {/* Call to Action Section - Muted Background */}
      <section className='w-full py-32 bg-muted/30 text-foreground relative overflow-hidden border-t border-border'>
