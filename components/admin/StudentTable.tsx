...
export function StudentTable({ students }: StudentTableProps) {
  ...
  const handleDeactivateConfirm = async () => {
    ...
    setIsSubmitting(true);
    try {
      await toggleStudentActiveStatus(
        statusModal.studentId,
      ...
      <table className='w-full text-left border-collapse'>
        ...
        <tbody className='text-zinc-300'>
          {students.map((student) => (
            <tr
              key={student.id}
              className='border-b border-zinc-900 hover:bg-white/5 transition-colors group'
            >
              <td
                className={`py-4 px-6 ${!student.isActive ? "opacity-50" : ""}`}
              >
                <button
              ...
              >
                <div className='flex items-center gap-2'>
                  <span
                    ...
                    style={{
                      backgroundColor:
                        student.rank.toLowerCase() === "black"
                          ? "#000"
                          : "#fff",
                              ...
                              <button
                                onClick={() => {
                                  setClassModal({
                                    ...
                                    studentId: student.id,
                                    studentName: student.name,
                                    currentClass: student.classSchedule,
                                  });
                                  setOpenMenuId(null);
                                }}
                                className='w-full px-4 py-2.5 text-left text-xs font-bold uppercase tracking-widest text-zinc-400 hover:bg-zinc-800 hover:text-white transition-colors flex items-center gap-3'
        ...
        >
          <form
            action={async (formData) => {
              ...
              try {
                ...
                const admissionNumber = formData.get(
                  "admissionNumber"
                ) as string;
                const email = formData.get("email") as string;
                await adminUpdateStudentProfile(editModal.student!.id, {
                  name,
                  admissionNumber,
                  email,
                });
                setNotification({
                  isOpen: true,
                  title: "SUCCESS",
                  message: "Personnel file updated.",
                  severity: "info",
                });
                setEditModal(null);
              } catch (e) {
          ...
          >
            <div>
              ...
              <input
                name='name'
                defaultValue={editModal.student.name}
            ...
            <div>
              ...
              <input
                ...
                defaultValue={editModal.student.admissionNumber}
                className='w-full bg-black border border-zinc-800 p-2 text-white font-bold'
              />
            </div>
            <div>
              ...
              <input
                name='email'
                defaultValue={editModal.student.email}
                className='w-full bg-black border border-zinc-800 p-2 text-white font-bold'
                required
                type='email'
              />
        ...
        >
          <div className='space-y-6'>
            <p className='text-zinc-400 text-sm leading-relaxed'>
              You are about to{" "}
              {statusModal.currentStatus ? "deactivate" : "reactivate"}{" "}
              <span className='text-white font-bold'>
                {statusModal.studentName}
              </span>
              .
            </p>
...
function ClassAssignmentForm({
  ...
  const [classes, setClasses] = useState<
    { id: string; name: string; day: string; time: string }[]
  >([]);
          ...
          >
            <div className='flex items-center gap-3'>
              <input
                ...
                name='classId'
                value={c.id}
                defaultChecked={c.name === currentClass}
